module ArticleHelper
  ...
  def read_button(article)
    ...
    if article.users.include?(current_user)
      ...
      link_to(read_article_path(article), method: :POST, class: 'btn btn-success', title: t('article.buttons.unread'), :'data-toggle' => 'tooltip', :'data-placement' => 'bottom', tooltip: true) do
        glyph(:ok)
      end
    end
  end
